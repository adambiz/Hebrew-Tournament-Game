...
function togglePowerUpsPanel() {
    ...
    if (powerupsPanel.classList.contains('hidden')) {
        ...
        if (hasPowerUps) {
            ...
            toast({
                title: "No Power-ups",
                description: "You don't have any power-ups. Purchase some between rounds!",
                variant: "destructive"
            });
        }
    } else {
...
function updatePowerUpButtonVisibility() {
    ...
    if (powerupButton) {
        ...
        if (hasAnyPowerUps) {
            ...
            powerupButton.setAttribute('aria-label', 'Open bonus menu');
            ...
            powerupButton.setAttribute('aria-label', 'No bonuses available yet');
            powerupButton.classList.remove('pulse-animation');
        }
    }
}
...
function applyDoublePointsPowerUp() {
    ...
    toast({
        ...
        description: "Points for this word will be doubled!",
        variant: "default"
    });
...
function applyLetterFilterPowerUp() {
    ...
    if (lettersToDisable.size > 0) {
        ...
        // Visual feedback
        toast({
            title: "Letter Filter Applied",
            description: `Filtered out ${lettersToDisable.size} similar-sounding letters for this word`,
            variant: "default"
        });
        ...
        toast({
            title: "Letter Filter",
            description: "No letters to filter for this word",
            variant: "default"
        });
...
function applySecondChanceRoundPowerUp() {
    ...
    // Visual feedback
    toast({
        title: "Second Chance Activated",
        description: "You now have an extra chance on all words in this round!",
        variant: "default"
    });
...
function applyEasierWordPowerUp() {
    ...
    if (currentLevel <= 1) {
        toast({
            title: "Cannot Make Word Easier",
            description: "Already at the easiest level of words!",
            variant: "destructive"
        });
    ...
    if (easierWords.length === 0) {
        toast({
            title: "Error",
            description: "Could not find an easier word",
            variant: "destructive"
        });
    ...
    let message = `New word: ${newWord.german} (Level ${targetLevel}, original points still awarded)`;
    
    if (newWord.isPhrase) {
        if (targetLevel === 5) {
            message = `Simplified to three-word sentence (Level ${targetLevel}, original points still awarded)`;
        } else if (targetLevel === 4) {
            message = `Simplified to two-word phrase (Level ${targetLevel}, original points still awarded)`;
        } else {
            message = `Simplified to shorter phrase (Level ${targetLevel}, original points still awarded)`;
        }
    } else {
        message = `Simplified to single word (Level ${targetLevel}, original points still awarded)`;
    }
    
    toast({
        title: "Word Made Easier",
        description: message,
        variant: "default"
    });
