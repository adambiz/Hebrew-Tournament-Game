...
function startNextWord() {
    ...
    document.getElementById('current-word-counter').textContent = 
        `Word ${gameState.currentWordIndex + 1} of ${gameState.roundWords.length}`;
...
function finalizeWordCheck(correctLetters, totalLetters, letterChecks) {
    ...
    announceWordUi(
        isPerfect
            ? `Perfect answer. ${finalPoints} points and ${coinsToAward} coins.`
            : `Answer submitted. ${finalPoints} points.`
    );
...
function handlePerfectWord(finalPoints, totalPossiblePoints, letterChecks, coinsEarned) {    
    ...
    let wordDescription = wordCount > 1 ? `${wordCount} words` : "word";
    let coinText = coinsEarned > 1 ? `${coinsEarned} coins` : "1 coin";
    
    // Show toast notification with original points info if easier word was used
    let description = `+${finalPoints} points, ${coinText}`;
    if (gameState.powerUpsActive.originalWord) {
        const originalLetters = gameState.powerUpsActive.originalWord.totalLetters;
        description = `+${finalPoints} points from original ${originalLetters}-letter ${wordDescription}, ${coinText}`;
    }
...
function handleImperfectWord(finalPoints, correctLetters, totalPossiblePoints, letterChecks) {
    ...
    let description = `+${finalPoints} points`;
    if (gameState.powerUpsActive.originalWord) {
        const originalLetters = gameState.powerUpsActive.originalWord.totalLetters;
        description = `+${finalPoints} points from original ${originalLetters}-letter word`;
    }
    
    // Show toast notification
    toast({
        title: finalPoints > 0 ? "כמעט!" : "טעות",
        description: description,
...
function showCelebrationEffect(type) {
    ...
    if (type === 'perfect') {
        ...
        // Add multiple celebratory emojis
        const emojis = ['🎉', '✨', '🎊', '👏', '⭐'];
        
        // Create emoji elements with different animations
        for (let i = 0; i < 8; i++) {
            ...
            emoji.style.animationDuration = `${0.8 + Math.random() * 0.6}s`; // Slightly faster but varied
    ...
    if (type === 'round-advance') {
        ...
        roundAdvanceText.textContent = 'Round Complete!';
